...
function newPropertyEpcVariantCopy(variant = requestedNewPropertyEpcVariant()) {
  const variants = {
    ...
    expired: {
      ...
      summary: "Demo match: EPC record appears expired. CMP would keep this open until it is checked, proof is added later, or an assessment is requested.",
    ...
    missing: {
      ...
      summary: "Demo match: CMP could not find a clear EPC signal. Book an assessment or add proof later before relying on the property file.",
...
const journeyUnknownQuestions = [
  ...
  },
  {
    ...
    options: [
      { id: "held", label: "I have a valid EICR", subtext: "CMP will ask for proof later.", effect: "EICR marked as held - proof needed later" },
      { id: "noProof", label: "I have it, but not with me", subtext: "Add proof to Evidence Vault later.", effect: "EICR evidence gap added" },
      { id: "expired", label: "I need to book or request one", subtext: "EICR action added.", effect: "EICR request action added" },
      { id: "unknown", label: "I don't know", subtext: "Keep as evidence gap.", effect: "EICR evidence gap kept open" }
    ]
  },
  {
    id: "alarms",
  ...
  },
  {
    ...
    options: [
      { id: "held", label: "Yes, I hold proof", subtext: "Add proof to Evidence Vault later.", effect: "Tenancy evidence marked as held - proof needed later" },
      { id: "noProof", label: "Yes, but I need to find proof", subtext: "Keep as evidence gap.", effect: "Tenancy evidence gap added" },
...
const journeyServiceCatalog = [
  ...
  },
  {
    ...
    routeTypes: ["prioritised", "legalMinimum", "riskProtected", "doneForMe"],
    bundleTags: ["legal essentials", "urgent only", "done for me"],
    whatItFixes: "Turns missing Electrical Safety evidence into a proof-later or requested-inspection route.",
  ...
  },
  {
    ...
    questions: [
      ...
      "Documents or proof to add later?"
    ]
  },
  {
  ...
  },
  {
    ...
    questions: ["Which documents do you already have?", "Any tenant communications?", "Any certificates arranged elsewhere?", "Preferred review priority?"]
  },
  {
    id: "pest-control",
    title: "Pest Control",
...
const guidedTourSteps = [
  ...
  },
  {
    ...
    body: "CMP combines found records, landlord answers and evidence gaps before showing the action plan.",
    primaryAction: "Build Property Intelligence",
    nextStep: "compliance-analysis",
    note: "Unknowns now become a property-specific compliance analysis.",
    callout: "This is the bridge from answers into scores, blockers, evidence gaps and monitoring.",
    guideCopy: "Run the analysis."
  },
  {
    ...
    body: "CMP shows the property profile, scores, risk level, blockers, missing evidence and future risks.",
    primaryAction: "Open Action Plan",
    nextStep: "action-plan",
    note: "The user should understand the analysis before seeing services or scenarios.",
    callout: "This follows the master journey: records and answers create analysis before action.",
  ...
  },
  {
    ...
    title: "Action Plan",
    body: "CMP groups urgent blockers, expiring items, missing evidence, future risks and opportunities, then recommends one next action.",
    primaryAction: "Prepare service request",
    nextStep: "take-action",
    note: "The action plan follows the buckets in the master journey.",
    callout: "The user should see why the recommended action exists before services appear.",
    guideCopy: "Use the recommended action."
  },
  {
    id: "take-action",
    section: "Take Action",
    stage: "action",
    screen: "takeAction",
    target: "take-action-primary",
    title: "Take Action",
    body: "CMP can help with proof, service requests, quotes, Ask CMP, reminders, communications and professional support. The demo follows one clear action.",
    primaryAction: "Prepare service request",
    nextStep: "monitoring",
    note: "Take Action should feel structured, not like a marketplace dump.",
    callout: "Supporting systems appear because the action plan produced a real property gap.",
    guideCopy: "Prepare one action."
  },
  {
    id: "monitoring",
    section: "Monitor & Update",
    stage: "monitor",
    screen: "monitoring",
    target: "monitoring-item",
    title: "Monitor & Update",
    body: "CMP keeps this property watched: expiries, law changes, evidence gaps, property changes and supplier follow-up stay visible.",
    primaryAction: "Finish main journey",
    nextStep: "scenario-handoff",
    note: "End on the recurring value.",
    callout: "The long-term value is monitoring: expiry dates, law change watch, evidence watch and follow-up.",
...
const guidedDemoStories = {
  "clean-property-check": {
    title: "Guided landlord compliance journey",
    scenarioId: "clean-property-match",
    proof: "CMP teaches one landlord how to add a property, run checks, answer unknowns, analyse compliance, act and monitor.",
    time: "2 minutes",
    motif: "match",
  ...
  "no-epc-found": {
    ...
    moments: [
      ...
      { name: "No EPC branch", stage: "confirmProperty", screen: "match", note: "Choose the property context.", callout: "CMP asks the minimum context needed to decide whether EPC is urgent, pre-let or proof can be added later." },
      ...
      { name: "Confirm unknowns", stage: "unknowns", screen: "unknowns", noEpcChoice: "advertised", answers: { occupancy: "advertised", propertyType: "house", occupants: "unknown", gas: "unknown", eicr: "unknown", alarms: "unknown", deposit: "none", tenancyDocs: "unknown", condition: "none", intent: "minimum" }, note: "Confirm only what the landlord can know.", callout: "CMP does not invent an EPC rating. The EPC stays unknown until a current certificate is requested or proof is added later." },
      { name: "Action plan", stage: "actionPlan", screen: "actionPlan", noEpcChoice: "advertised", note: "Point to EPC request and Add Proof Later routes.", callout: "The action plan now has a practical route: prepare an EPC request, add proof later, or continue with a warning." },
      { name: "Prepare EPC request", stage: "action", screen: "workspace", workspaceTab: "services", noEpcChoice: "advertised", servicePlan: "urgent", note: "Recommended route should be EPC Assessment, not an improvement plan.", callout: "This is a missing-evidence route. CMP recommends an EPC assessment request or proof added later, not confident improvement advice." },
      { name: "Workspace evidence watch", stage: "vault", screen: "workspace", workspaceTab: "evidence", noEpcChoice: "advertised", note: "End with the evidence gap still visible.", callout: "No EPC remains monitored in the property workspace until evidence is found, proof is added later or an assessment is requested." },
      { name: "Ask CMP explains the missing record", stage: "action", screen: "workspace", workspaceTab: "ask", noEpcChoice: "advertised", note: "Ask CMP should explain the missing EPC without pretending it has live certainty.", callout: "The answer should say this is a simulated missing-record check and that a current EPC may need booking or proof added later." },
...
const nickScenarioExplorerCards = [
  {
    id: "done-for-me-plan",
    title: "Done-for-me compliance plan",
    purpose: "Commercial managed route.",
    finds: "Service bundle",
    gap: "Proof still pending",
    action: "Request quotes",
    firstClick: "Choose done-for-me",
    badge: "Recommended first"
  },
  {
    id: "damp-mould-enforcement",
    title: "Damp, mould or enforcement",
    purpose: "Condition evidence timeline.",
    finds: "Repair context",
    gap: "Evidence trail",
    action: "Survey/message",
    firstClick: "Pick damp/mould",
    badge: "Recommended risk/evidence"
  },
  {
    id: "no-epc-found",
    title: "No EPC found",
    purpose: "Missing public record.",
    finds: "No EPC signal",
    gap: "EPC proof needed",
    action: "Prepare assessment",
    firstClick: "Run checks"
  },
  {
    id: "epc-expired-mees-risk",
    title: "EPC expired / MEES risk",
    purpose: "Energy evidence risk.",
    finds: "Weak EPC record",
    gap: "Review before relying",
    action: "EPC route",
    firstClick: "Review found data"
  },
  {
    id: "hmo-licensing-risk",
    title: "HMO or licensing risk",
    purpose: "Occupancy changes route.",
    finds: "Shared-house signals",
    gap: "Licensing check",
    action: "Fire/licence review",
    firstClick: "Answer occupancy"
  },
  {
    id: "portfolio-landlord-preview",
    title: "Portfolio landlord preview",
    purpose: "Scale-up model.",
    finds: "Cross-property risk",
    gap: "Priorities by property",
    action: "Portfolio watch",
    firstClick: "Show risk plan",
    badge: "Secondary"
  },
  {
    id: "clean-property-check",
    title: "Clean property",
    purpose: "Normal landlord journey.",
    finds: "Address + EPC + basics",
    gap: "Confirm unknowns",
    action: "Monitor",
    firstClick: "Check My Property",
    badge: "Secondary"
  }
];
...
const journeyScanOutcomes = {
  valid: {
    ...
    status: "accepted",
    confidence: "High",
    body: "Demo proof marked as held. Score improves, expiry reminder is created if relevant, and the linked compliance item updates."
  },
  unclear: {
    label: "Unclear",
    status: "flagged",
  ...
  expired: {
    ...
    status: "flagged",
    confidence: "High",
    body: "Demo proof marked as expired. Replacement service remains recommended."
  },
  wrong_property: {
...
const journeyScanSteps = [
  "Proof placeholder opened",
  "Reading demo details",
  "Checking address",
  "Extracting dates",
  "Matching to compliance rule",
  "Checking expiry",
  "Updating demo Evidence Vault"
];
...
const journeyMonitoringCatalog = [
  ...
  { id: "gas-renewal-monitor", type: "Expiry reminder", title: "Gas Safety renewal", dueDate: "Unknown until certificate proof is added", urgency: "High", linkedActionId: "gas-safety", description: "Create renewal tracking after evidence is added or a request is prepared." },
  { id: "eicr-renewal-monitor", type: "Expiry reminder", title: "EICR renewal", dueDate: "Unknown until EICR proof is added", urgency: "High", linkedActionId: "eicr", description: "Track Electrical Safety renewal after the current report is known." },
  ...
  { id: "inspection-due", type: "Condition review", title: "Inspection due", dueDate: "Next 30 days", urgency: "Medium", linkedActionId: "condition-inspection", description: "Schedule or add proof of a condition inspection later." },
...
function createJourneyPropertyBrain(scenarioId = "clean-property-match") {
  ...
  return {
    ...
    AutoCheckResults: {
      ...
      epcRecommendations: epcFound ? ["Improve loft insulation", "Add heating controls / TRVs", "Review low-energy lighting"] : ["Prepare EPC assessment request"],
    ...
    ComplianceEvidence: {
      epc: createJourneyEvidenceItem(epcFound ? (scenario.epcExpired ? "expired" : "found") : "missing", epcFound ? "Official record signal" : "No proof added yet", epcFound ? (scenario.epcExpiry || "14 March 2031") : "Unknown", "Simulated EPC lookup", epcFound ? "High" : "Low", scenario.epcExpired ? "Prepare EPC assessment request or review MEES position" : epcFound ? "Review record" : "Prepare EPC assessment or add proof later"),
      gasSafety: createJourneyEvidenceItem(gasUnknown ? "unknown" : "missing", "No proof added yet", "Unknown", "Landlord answer needed", gasUnknown ? "Low" : "Medium", "Confirm gas position and add proof later or request Gas Safety"),
      eicr: createJourneyEvidenceItem(eicrMissing ? "missing" : "found", eicrMissing ? "No proof added yet" : "Demo proof marked as held", eicrMissing ? "Unknown" : "11 May 2031", eicrMissing ? "Landlord proof needed later" : "Demo evidence", eicrMissing ? "Low" : "High", eicrMissing ? "Prepare EICR request or add proof later" : "Monitor renewal"),
      ...
      rightToRent: createJourneyEvidenceItem("unknown", "No proof added yet", "Unknown", "Landlord answer needed", "Low", "Add Right to Rent proof later or confirm evidence status"),
      tenancyDocs: createJourneyEvidenceItem("unknown", "No proof added yet", "Unknown", "Landlord answer needed", "Low", "Add tenancy proof later or review"),
      inspectionReports: createJourneyEvidenceItem("missing", "No inspection report", "Unknown", "Landlord proof needed later", "Low", "Add inspection proof later or prepare inspection request"),
      ...
      insurance: createJourneyEvidenceItem("unknown", "No proof added yet", "Unknown", "Landlord proof needed later", "Low", "Add insurance proof later"),
...
function createInitialJourneyState(scenarioId = "clean-property-match") {
  ...
  const state = {
    ...
    dependencyNotice: "",
    pendingJumpTarget: "",
    jumpAheadConfirmed: false,
    timelineEvents: [
      { id: "journey-ready", title: "Property check ready", body: "Demo mode started with simulated data.", type: "Setup", time: "Now" }
    ],
    evidenceVault: [],
    monitoringItems: [],
    askHistory: [],
...
function getJourneyBridgeAssistantResponse(prompt) {
  ...
  if (labsState.currentView === "journeyOs") {
    return `CMP is building the ${route} route for ${address}. It checks simulated records first, then asks only landlord-only unknowns before returning to the workspace.`;
  }
  if (labsState.currentView === "complianceCentre") {
    return `Compliance Centre is now reading the Property Intelligence profile: ${urgent.length} urgent blocker${urgent.length === 1 ? "" : "s"}, ${missing.length} evidence gap${missing.length === 1 ? "" : "s"}, and ${state.serviceBasket.length} demo service item${state.serviceBasket.length === 1 ? "" : "s"}.`;
  }
  if (labsState.currentView === "evidenceVault") {
    return state.evidenceVault.length
      ...
      : `Evidence Vault is waiting for proof to be added later for ${address}. Start with ${missing[0]?.title || "EICR, Gas Safety or tenancy evidence"} or use the prototype placeholder.`;
  }
...
function serviceConfirmationFor(status, item, services = [journeyServiceById(item.serviceId)]) {
  const referencePrefix = status === "quote_requested" ? "CMP-QUOTE" : status === "booked" ? "CMP-REQ" : status === "pending" ? "CMP-JOB" : status === "completed" ? "CMP-EVID" : "CMP-BASKET";
  ...
  const titles = {
    ...
    booked: "Service request prepared",
    pending: "Service marked in progress",
    completed: "Service completed",
    deferred: "Service saved for later"
  };
  const bodies = {
    added: "CMP added this to the demo basket. Risk stays visible until the service is quoted, requested, completed or proof is added later.",
    quote_requested: "CMP prepared demo quote details. Compliance risk remains open until the quote becomes a request or evidence is accepted.",
    booked: "CMP prepared a demo service request, added pending evidence, updated monitoring and logged the next step.",
    pending: "CMP marked the demo job in progress and kept the evidence follow-up visible.",
  ...
  return {
    ...
    body: bodies[status] || "The demo service state was updated.",
    reference,
    status,
...
function guidedTourStepIndexForLiveState() {
  ...
  if (state.modalMode === "service-confirmation") {
    return guidedTourSteps.findIndex((step) => step.id === "take-action");
  }
  ...
  if (state.screen === "brain") return guidedTourSteps.findIndex((step) => step.id === "compliance-analysis");
  if (state.screen === "actionPlan") return guidedTourSteps.findIndex((step) => step.id === "action-plan");
  if (state.screen === "takeAction") return guidedTourSteps.findIndex((step) => step.id === "take-action");
  if (state.screen === "monitoring") return guidedTourSteps.findIndex((step) => step.id === "monitoring");
  if (state.screen === "workspace" && state.workspaceTab === "overview") return guidedTourSteps.findIndex((step) => step.id === "overview-tour");
  if (state.screen === "workspace" && state.workspaceTab === "compliance") return guidedTourSteps.findIndex((step) => step.id === "compliance-tour");
  if (state.screen === "workspace" && state.workspaceTab === "evidence") return guidedTourSteps.findIndex((step) => step.id === "evidence-vault");
  if (state.screen === "workspace" && state.workspaceTab === "services") return guidedTourSteps.findIndex((step) => step.id === "services-tour");
  if (state.screen === "workspace" && state.workspaceTab === "timeline") return guidedTourSteps.findIndex((step) => step.id === "timeline-tour");
  ...
  if (state.screen === "workspace") return guidedTourSteps.findIndex((step) => step.id === "overview-tour");
  return -1;
}
...
function guidedRecommendedPlanForStory(storyId = guidedDemoState().activeStoryId) {
  const moment = currentGuidedMoment();
  if (moment?.servicePlan) {
    return moment.servicePlan;
  }
  if (moment?.route === "doneForMe" || storyId === "done-for-me-plan") {
    return "quotes";
  }
  if (["no-epc-found", "hmo-licensing-risk"].includes(storyId)) {
    return "legal";
  }
  if (storyId === "epc-expired-mees-risk") {
    return "future";
  }
  if (["damp-mould-enforcement", "portfolio-landlord-preview"].includes(storyId)) {
    return "risk";
  }
  return "legal";
}
...
function guidedAskPromptForStory(storyId = guidedDemoState().activeStoryId) {
  const prompts = {
    ...
    "damp-mould-enforcement": "What should I say to the tenant?",
    "done-for-me-plan": "What should I do after requesting quotes?",
    "portfolio-landlord-preview": "What changed in this Property Intelligence profile?"
  };
  return prompts[storyId] || "What should I do first?";
}

function currentGuidedAction() {
  const demo = guidedDemoState();
  if (!demo.enabled) {
    return null;
  }
  ...
  if (demo.mode === "landing" && !demo.activeStoryId) {
    ...
    return {
      ...
      body: "Follow the guided story first. You can stress-test other landlord situations afterwards.",
      why: "This click shows the full Add Property to Monitoring spine without presenter controls."
    };
  }
  ...
  if (state.modalMode === "service-confirmation") {
    const step = guidedTourStepById("services-tour");
    return {
      target: step.target,
      title: step.title,
      body: step.body,
  ...
  if (state.screen === "start") {
    return {
      target: "start-check",
      title: "Click Check My Property",
      body: "Start with the property. CMP begins with an address, not a long form.",
      why: moment.note || story.proof
    };
  }
  ...
  if (state.screen === "add") {
    return {
      target: "run-auto-checks",
      title: "Run the simulated checks",
      body: "Run the demo checks. In the real product this would connect address, EPC and property data.",
      why: moment.note || "Smart Search checks what CMP can find before asking the landlord anything."
    };
  }

  if (state.screen === "autoChecks" && state.autoCheckStep >= journeyAutoCheckSteps.length) {
    return {
      target: "auto-review",
      title: "Review what CMP found",
      body: "Smart Search has finished. Continue into the real review step rather than skipping the story.",
      why: "Reviewing found data keeps the demo in the product journey."
    };
  }
  ...
  if (state.screen === "match") {
    return {
      target: "confirm-match",
      title: "Confirm the property context",
      body: "Review what CMP found before answering anything yourself.",
      why: "CMP separates found data from landlord-only unknowns."
    };
  }
  ...
  if (state.screen === "review") {
    return {
      target: "review-found-data",
      title: "Move into landlord-only unknowns",
      body: "CMP separates found data from landlord-only unknowns.",
      why: moment.note || "This keeps the product from feeling like a giant intake form."
    };
  }
  ...
  if (state.screen === "unknowns") {
    ...
    if (!question) {
      return {
        target: "build-brain",
        title: "Build the Property Intelligence profile",
        body: "The unknowns are handled. Create the profile so CMP can turn gaps into next steps.",
        why: "This is the bridge from answers into evidence, actions and monitoring."
      };
    }
    return {
      target: "recommended-answer",
      title: "Choose the highlighted answer",
      body: "Choose this answer to follow the current scenario path. You can still pick another answer.",
      why: question.id === "occupants" && isAdvertisedOrPreLet(state)
        ? "This is expected letting setup, not a contradictory live-occupancy claim."
        : moment.note || question.why
    };
  }

  if (state.screen === "brain" && state.brainStep >= journeyBrainSteps.length) {
    return {
      target: "open-action-plan",
      title: "Open the action plan",
      body: "CMP has combined records, answers and gaps into a compliance analysis. Open the action plan next.",
      why: "This is where analysis becomes practical landlord action."
    };
  }

  if (state.screen === "actionPlan") {
    return {
      target: "action-plan-primary",
  ...
  if (state.screen === "workspace" && state.workspaceTab === "evidence") {
    return {
      target: "evidence-gap",
      title: "Review the evidence gap",
      body: "The Evidence Vault becomes the source of truth.",
      why: moment.note || "Evidence stays attached to the property until proof is accepted or the gap is resolved.",
      autoAdvance: false
    };
  }

  if (state.screen === "workspace" && state.workspaceTab === "ask") {
    return {
      target: "ask-prompt",
      title: "Ask CMP with this prompt",
      body: "Ask CMP explains the property using the same evidence and answers.",
      why: moment.note || "This keeps the assistant property-specific rather than generic.",
      autoAdvance: false
    };
  }

  if (state.screen === "workspace" && state.workspaceTab === "monitoring") {
    return {
      target: "monitoring-item",
      title: "Set the follow-up",
      body: "This is the ongoing subscription value - CMP watches the property over time.",
      why: moment.note || "Monitoring keeps renewal dates, evidence watch and annual review visible."
    };
  }
  ...
  if (state.screen === "workspace") {
    return {
      ...
      title: "Click the highlighted follow-through",
      body: "No supplier is contacted in this prototype - this shows the future service path.",
      why: moment.note || `${story.title} should end in a practical gap, action and monitoring path.`
    };
  }
...
function currentDemoGuide(action = currentGuidedAction()) {
  ...
  const guides = {
    ...
    "build-brain": { pose: "thinking", copy: "Now CMP builds the property profile.", direction: "right" },
    "open-workspace": { pose: "point", copy: "Open the workspace next.", direction: "right" },
    "open-action-plan": { pose: "point", copy: "Now open the action plan.", direction: "right" },
    "workspace-tab-compliance": { pose: "point", copy: "Next: Compliance.", direction: "right" },
    "workspace-tab-evidence": { pose: "point", copy: "Next: Evidence.", direction: "right" },
    "workspace-tab-services": { pose: "point", copy: "Next: Services.", direction: "right" },
    "workspace-tab-timeline": { pose: "point", copy: "Next: Timeline.", direction: "right" },
    "workspace-tab-ask": { pose: "point", copy: "Next: Ask CMP.", direction: "right" },
    "action-plan-primary": { pose: "point", copy: "This is where CMP turns gaps into next steps.", direction: "right" },
    "take-action-primary": { pose: "point", copy: "Prepare one action before moving to monitoring.", direction: "right" },
...
function guidedTargetAttrs(target) {
  ...
  return ` data-guided-target="${escapeHtml(target)}" aria-describedby="guidedCoachmarkTitle"`;
}

function clickedGuidedTarget(event) {
  return event.target.closest("[data-guided-target]")?.dataset.guidedTarget || "";
}

function isCurrentGuidedTarget(target) {
  const action = currentGuidedAction();
...
function advanceGuidedProductTarget(target, { applyMoment = false, render = false, verifyCurrent = true } = {}) {
  ...
  const nextIndex = Math.min(story.moments.length - 1, demo.activeMomentIndex + 1);
  if (nextIndex === demo.activeMomentIndex) {
    return false;
  }
  if (applyMoment) {
...
function advanceGuidedMomentToNextScreen(screen) {
  const demo = guidedDemoState();
  const story = currentGuidedStory();
  if (!demo.enabled || !story) {
    return false;
  }
  const nextIndex = story.moments.findIndex((moment, index) => index > demo.activeMomentIndex && moment.screen === screen);
  if (nextIndex < 0) {
    return false;
  }
  demo.activeMomentIndex = nextIndex;
...
function syncGuidedDemoClass() {
  const demo = guidedDemoState();
  const state = journeyState();
  const askStepActive = Boolean(demo.enabled && !demo.freeExplore && state.screen === "workspace" && state.workspaceTab === "ask");
  document.body.classList.toggle("journey-guided-active", Boolean(demo.enabled));
  document.body.classList.toggle("journey-guided-ask-active", askStepActive);
...
function resetJourneyStateForGuidedStory(storyId) {
  ...
  addTimelineEvent({
    ...
    body: "Guided demo reset this property compliance profile with simulated data.",
    type: "Guided demo"
  });
  return state;
}

function answerOptionLabel(questionId, answerId) {
  const question = journeyUnknownQuestions.find((item) => item.id === questionId);
  if (Array.isArray(answerId)) {
    return conditionAnswerLabels(answerId).join(", ");
  }
  return question?.options.find((option) => option.id === answerId)?.label || answerId || "";
}

function isCurrentlyVacant(state = journeyState()) {
  return state.answers?.occupancy === "vacant";
}

function isAdvertisedOrPreLet(state = journeyState()) {
  return ["advertised", "vacant"].includes(state.answers?.occupancy);
}

function isSkippedJourneyQuestion(questionId, state = journeyState()) {
  return questionId === "occupants" && isCurrentlyVacant(state);
}

function journeyQuestionDisplay(question, state = journeyState()) {
  if (question.id === "occupants" && state.answers?.occupancy === "advertised") {
    return {
      title: "Expected occupancy when let?",
      why: "If you plan to let it, expected occupant count can change HMO, licensing and management-standard risk."
    };
  }
  if (question.id === "occupants" && isCurrentlyVacant(state)) {
    return {
      title: "How many people live there?",
      why: "Skipped because the property is currently vacant. CMP will ask again if the property is let or marketed with a specific occupancy plan."
    };
  }
  return {
...
function purgeRouteEffects(patterns) {
  const state = journeyState();
  state.branchEffects = (state.branchEffects || []).filter((effect) => !patterns.some((pattern) => pattern.test(effect)));
}

function cleanupJourneyAnswerDependencies(changedQuestionId = "") {
  const state = journeyState();
  const brain = state.propertyBrain;
  const notices = [];

  if (changedQuestionId === "occupancy" && isCurrentlyVacant(state)) {
    if (state.answers.occupants) {
      delete state.answers.occupants;
      notices.push("We've removed the later occupant-count answer because the property is currently vacant.");
    }
    brain.TenancyProfile.occupantCount = "Not applicable - currently vacant";
    brain.TenancyProfile.householdCount = "Not applicable - currently vacant";
    if (state.answers.propertyType !== "room") {
      brain.PropertyIdentity.hmoRiskFlag = Boolean(journeyDemoScenarios[state.scenarioId]?.hmoRiskFlag);
    }
    purgeRouteEffects([
      /HMO/i,
      /occupancy confidence/i,
      /licensing check added/i,
      /licensing route/i,
      /high occupancy/i
    ]);
  }

  if (changedQuestionId === "propertyType" && state.answers.propertyType !== "room") {
    const occupantHmo = ["threeFour", "fivePlus"].includes(state.answers.occupants);
    brain.PropertyIdentity.hmoRiskFlag = Boolean(journeyDemoScenarios[state.scenarioId]?.hmoRiskFlag || occupantHmo);
  }

  if (changedQuestionId === "occupants" && isCurrentlyVacant(state)) {
    delete state.answers.occupants;
    brain.TenancyProfile.occupantCount = "Not applicable - currently vacant";
    notices.push("Occupant count is not needed while the property is marked vacant.");
  }

  if (notices.length) {
    state.dependencyNotice = notices[0];
    state.branchEffects.unshift(notices[0]);
  }
}

function nextApplicableUnknownIndex(index, state = journeyState()) {
  let nextIndex = index;
  while (nextIndex < journeyUnknownQuestions.length && isSkippedJourneyQuestion(journeyUnknownQuestions[nextIndex].id, state)) {
    nextIndex += 1;
  }
  return nextIndex;
}
...
function applyGuidedAnswerSet(answers = {}, unknownIndex = null) {
  ...
  orderedAnswers.forEach(([questionId, answerId]) => {
    if (!isSkippedJourneyQuestion(questionId, journeyState())) {
      answerUnknown(questionId, answerId, { silent: true });
    }
  });

  const state = journeyState();
  state.unknownIndex = nextApplicableUnknownIndex(unknownIndex ?? (orderedAnswers.length ? journeyUnknownQuestions.length : state.unknownIndex), state);
  state.actionPlan = buildJourneyActionPlan(state);
  state.serviceRecommendations = buildServiceRecommendations(state);
}
...
function guidedUnknownIndexForMoment(moment) {
  ...
  if (!moment?.answers || moment.screen !== "unknowns") {
    return null;
  }
  const name = String(moment.name || "").toLowerCase();
  const preferredQuestionId = name.includes("intent") ? "intent"
    : name.includes("condition") || name.includes("damp") ? "condition"
    : name.includes("occupant") ? "occupants"
    : name.includes("property type") || name.includes("shared-house") ? "propertyType"
    : "occupancy";
  const preferredIndex = journeyUnknownQuestions.findIndex((question) => question.id === preferredQuestionId);
...
function prepareStateForGuidedTourStep(state, step) {
  ...
  const analysisIndex = guidedTourSteps.findIndex((item) => item.id === "compliance-analysis");
  const actionPlanIndex = guidedTourSteps.findIndex((item) => item.id === "action-plan");
  const takeActionIndex = guidedTourSteps.findIndex((item) => item.id === "take-action");
  ...
  if (stepIndex >= analysisIndex) {
    state.brainStep = journeyBrainSteps.length;
    state.actionPlan = buildJourneyActionPlan(state);
    state.serviceRecommendations = buildServiceRecommendations(state);
  }

  if (stepIndex >= actionPlanIndex) {
    state.actionPlan = buildJourneyActionPlan(state);
    state.serviceRecommendations = buildServiceRecommendations(state);
  }

  if (stepIndex >= takeActionIndex && !state.serviceBasket.length) {
    const action = recommendedMainJourneyAction();
    const service = journeyServiceForAction(action.id);
    ensureServiceBasketItem(service.id, "added");
  }
...
function applyGuidedMoment(storyId, momentIndex) {
  ...
  if (moment.answers) {
    applyGuidedAnswerSet(moment.answers, guidedUnknownIndexForMoment(moment));
  }
  if (moment.route) {
  ...
  if (moment.brainComplete) {
    ...
    state.serviceRecommendations = buildServiceRecommendations(state);
    addTimelineEvent({
      title: "Property compliance profile created",
...
function buildJourneyActionPlan(state = journeyState()) {
  ...
  if (!auto.epcFound || scenario.urgentMees || scenario.epcExpired) {
    ...
    const body = scenario.urgentMees
      ...
      : scenario.epcExpired
        ...
        : "No clear EPC record was found. Prepare an EPC assessment request or add existing proof later before marketing or relying on the file.";
    ...
    recommendedServices.push(journeyAction("book-epc", "Prepare EPC assessment request", "Prepare an EPC assessment request or add existing proof to the Evidence Vault later.", "Recommended Services", ["prioritised", "legalMinimum", "futureProof", "doneForMe"], "high"));
  }

  if (evidence.gasSafety.status !== "found" && answers.gas !== "no") {
    urgentLegalBlockers.push(journeyAction("gas-safety", "Gas Safety Certificate missing or unconfirmed", "CMP cannot see a current Gas Safety Certificate. Confirm gas status, add proof later or prepare a check request.", "Urgent Legal Blockers", ["prioritised", "legalMinimum", "riskProtected", "doneForMe"], "high"));
    recommendedServices.push(journeyAction("gas-service", "Gas Safety check request", "Confirm appliances and prepare a gas safety check request if gas is present.", "Recommended Services", ["prioritised", "legalMinimum", "doneForMe"], "high"));
  }

  if (evidence.eicr.status === "missing" || ["expired", "unknown"].includes(answers.eicr)) {
    urgentLegalBlockers.push(journeyAction("eicr", "EICR evidence missing", "Add proof later or prepare an inspection request so Electrical Safety is no longer an unknown.", "Urgent Legal Blockers", ["prioritised", "legalMinimum", "riskProtected", "doneForMe"], "high"));
    recommendedServices.push(journeyAction("book-eicr", "Prepare EICR request", "Prepare an Electrical Safety inspection request and add proof to the Evidence Vault later.", "Recommended Services", ["prioritised", "legalMinimum", "doneForMe"], "high"));
  } else if (evidence.eicr.status === "held_needs_proof") {
    missingEvidence.push(journeyAction("eicr-proof", "EICR proof needed later", "Landlord says a valid EICR exists. CMP keeps proof as an evidence gap until it is added later.", "Missing Evidence", ["prioritised", "riskProtected", "doneForMe"], "medium"));
  }
  ...
  const hmoSupportedByScenario = Boolean(scenario.hmoRiskFlag);
  const hmoSupportedByCurrentAnswers = ["occupied", "advertised"].includes(answers.occupancy) && (["room"].includes(answers.propertyType) || ["threeFour", "fivePlus"].includes(answers.occupants));
  if (hmoSupportedByScenario || hmoSupportedByCurrentAnswers) {
  ...
  [
    ["right-to-rent", "Right to Rent evidence", "Add or confirm Right to Rent evidence for the tenancy file."],
    ["tenancy-docs", "Tenancy agreement / written terms", "Add proof that required tenancy information was provided."],
    ...
    ["insurance", "Insurance unknown", "Add the landlord insurance schedule later or mark it as handled elsewhere."]
  ...
  if (hasCondition) {
    ...
    if (damp) {
      conditionRisks.push(journeyAction("damp-condition-risk", "Damp, mould, heating or leak risk", "Prepare a damp/mould survey request or create repair evidence so this does not sit as an untracked complaint.", "Condition Risks", ["prioritised", "riskProtected", "doneForMe"], "medium"));
    ...
    if (unknownCondition || (!council && !damp && !pests && !electrical)) {
      conditionRisks.push(journeyAction("condition-inspection", "Known or uncertain property condition risk", "Prepare a condition inspection request or add repair evidence later.", "Condition Risks", ["prioritised", "riskProtected", "doneForMe"], "medium"));
...
function upsertJourneyEvidenceFromService(serviceId, status) {
  ...
  const evidence = {
    ...
    fakeScanResult: completed ? "Generated by completed demo service" : "Pending from demo request",
...
function applyServiceLifecycleSideEffects(serviceId, status, source = "Service basket") {
  if (["booked", "pending", "completed"].includes(status)) {
    ...
    addTimelineEvent({
    title: status === "completed" ? "Evidence generated from service" : "Evidence pending from service request",
...
function updateServiceStatus(serviceId, status, source = "Service basket") {
  ...
  const statusCopy = {
    ...
    quote_requested: "prepared as a demo quote request",
    booked: "moved to requested/pending in the demo basket",
...
function removeServiceFromBasket(serviceId) {
  ...
  addTimelineEvent({
    ...
    body: `${service.title} removed from the demo basket. Linked risk remains visible in the action plan.`,
    type: "Services"
  });
  state.serviceRecommendations = buildServiceRecommendations(state);
  state.propertyBrain.Scores = recalculateJourneyScores(state, state.actionPlan);
...
function applyServicePlan(action) {
  ...
  state.activeConfirmation = {
    ...
    body: action === "quotes"
      ? "CMP prepared quote requests from the current property compliance profile. Compliance risk remains open until quotes become requests or evidence is accepted."
      : action === "concierge"
        ? "CMP grouped urgent compliance, evidence recovery and monitoring into a demo concierge route. No supplier was contacted."
        : `${selected.length} service${selected.length === 1 ? "" : "s"} moved into the demo ${serviceLifecycleLabel(status).toLowerCase()} state.`,
    planAction: action
  };
  addTimelineEvent({
...
function startFakeUploadScan(documentTypeId) {
  ...
  addTimelineEvent({
    title: "Proof placeholder reviewed",
    body: `${journeyDocumentTypes.find((item) => item.id === documentTypeId)?.label || "Document"} entered the demo evidence preview. No file was uploaded.`,
...
function completeFakeScan() {
  ...
  const evidence = {
    ...
    title: doc?.label || "Evidence placeholder",
  ...
  if (scanner.outcomeId === "valid") {
    if (scanner.documentTypeId === "epc") {
      ...
      state.propertyBrain.AutoCheckResults.epcFound = true;
      state.propertyBrain.AutoCheckResults.epcRecordStatus = "EPC proof marked as held in demo";
    }
    ...
    if (scanner.documentTypeId === "council-letter") {
      state.propertyBrain.TenancyProfile.councilContactStatus = "Council evidence marked as held";
    }
  ...
  if (scanner.outcomeId === "expired") {
    state.branchEffects.unshift(`${evidence.title} marked expired - replacement action remains visible`);
    state.monitoringItems.unshift({
      ...
      description: "CMP marked the demo proof as expired and keeps the replacement route visible because expired evidence is not solved.",
      status: "watching"
    });
  }
...
function generateAskCmpResponse(prompt) {
  ...
  const requested = state.serviceBasket.filter((item) => ["booked", "pending"].includes(item.status));
  ...
  if (prompt.includes("request change") || prompt.includes("booking change")) {
    body = requested.length
      ? `Your demo requests moved ${requested.map((item) => item.title).join(", ")} into a supplier-ready preview state. CMP also added pending evidence and monitoring follow-ups, but the risk is not solved until proof is accepted or the service is completed.`
      : "No request has been prepared yet. Add a service or use Prepare legal essentials to create pending evidence and monitoring.";
  } else if (prompt.includes("waiting on a service")) {
    body = requested.length || quotes.length
      ? `CMP is waiting on ${[...requested, ...quotes].map((item) => `${item.title} (${serviceLifecycleLabel(item.status)})`).join(", ")}. Quote requests do not close compliance risk; completed services or accepted proof do.`
      : "Nothing is waiting on a service yet. Your next step is to prepare a request, request a quote, or add proof later.";
  } else if (prompt.includes("mark complete")) {
    body = requested.length
      ? `You can mark ${requested[0].title} in progress or complete from the Services tab. Completion will generate simulated evidence and refresh Evidence Vault and Monitoring.`
      : completed.length
        ? `${completed[0].title} is already complete in the prototype. View generated evidence or set renewal monitoring.`
        : "There is not a requested service ready to complete yet.";
  } else if (prompt.includes("requesting quotes")) {
    body = quotes.length
      ? `CMP has prepared quote requests for ${quotes.map((item) => item.title).join(", ")}. Your risk remains open until a quote is accepted, a request is completed, or proof is added later.`
    ...
    body = `${missing.length} action-plan evidence gap${missing.length === 1 ? "" : "s"} remain. ${pendingEvidence.length ? `${pendingEvidence.length} evidence item${pendingEvidence.length === 1 ? " is" : "s are"} pending from demo requests.` : "No pending service evidence has been created yet."}`;
  ...
  } else if (prompt.includes("book") || prompt.includes("request")) {
    body = `Prepare ${buildServiceRecommendations(state).slice(0, 3).map((service) => service.title).join(", ")} first. These map directly to the current risk and evidence gaps.`;
  ...
  } else if (prompt.includes("lowest-effort") || prompt.includes("cheapest")) {
    body = "The lowest-effort route is to handle legal essentials first: EPC if missing, Gas Safety if gas is present, EICR, alarms, deposit evidence and licensing check.";
  ...
  const basketCopy = state.serviceBasket.length ? ` ${state.serviceBasket.length} service item${state.serviceBasket.length === 1 ? "" : "s"} are already in the demo basket, including ${requested.length} requested/in-progress and ${quotes.length} quote request${quotes.length === 1 ? "" : "s"}.` : " No service basket has been started yet.";
  const response = {
    id: `ask-${Date.now()}`,
    prompt,
    body: `${propertyContext} ${body}${basketCopy}`,
    actions: ["Prepare Gas Safety request", "Prepare EICR request", "Add proof later", "Add legal essentials to basket", "Generate tenant message", "Set reminder", "View evidence gaps", "Escalate placeholder"]
  };
...
function completeGuidedMainActionPath() {
  const state = journeyState();
  const action = recommendedMainJourneyAction();
  const service = journeyServiceForAction(action.id);
  ensureServiceBasketItem(service.id, "booked");
  upsertJourneyEvidenceFromService(service.id, "booked");
  addTimelineEvent({
    title: "Guided service request prepared",
    body: `${service.title} was prepared from the recommended action: ${action.title}. No supplier was contacted.`,
    type: "Take action"
  });
  state.branchEffects.unshift("One recommended action prepared from the compliance plan");
  state.actionPlan = buildJourneyActionPlan(state);
  state.serviceRecommendations = buildServiceRecommendations(state);
...
function finishGuidedMainJourney() {
  const demo = guidedDemoState();
  demo.activeStoryId = "";
  demo.activeMomentIndex = 0;
  demo.mode = "landing";
...
function handleNoEpcChoice(choice) {
  ...
  const labels = {
    ...
    upload: "EPC proof held - add later",
  ...
  brain.ComplianceEvidence.epc.status = choice === "upload" ? "held_needs_proof" : "missing";
  if (carriedOccupancy) {
  ...
  state.branchEffects.unshift(choice === "vacant" ? "EPC request before marketing added" : choice === "upload" ? "EPC proof needed later" : "EPC assessment request added");
  state.actionPlan = buildJourneyActionPlan(state);
  addTimelineEvent({
...
function answerUnknown(questionId, answerId, options = {}) {
  const state = journeyState();
  if (questionId === "condition" && Array.isArray(answerId)) {
    answerConditionUnknown(answerId);
    return;
  }
  if (isSkippedJourneyQuestion(questionId, state)) {
    state.dependencyNotice = "Skipped occupant count because the property is currently vacant.";
    state.unknownIndex = nextApplicableUnknownIndex(state.unknownIndex + 1, state);
    renderJourneyOsState();
    return;
  }
  const question = journeyUnknownQuestions.find((item) => item.id === questionId);
  ...
  if (questionId === "occupancy") {
    ...
    cleanupJourneyAnswerDependencies("occupancy");
  }
  if (questionId === "propertyType") {
    ...
    cleanupJourneyAnswerDependencies("propertyType");
  }
  if (questionId === "occupants") {
    ...
    cleanupJourneyAnswerDependencies("occupants");
  }
  if (questionId === "gas") {
    brain.ComplianceEvidence.gasSafety.status = answerId === "no" ? "not_applicable" : answerId === "unknown" ? "unknown" : "missing";
  }
  if (questionId === "eicr") {
    brain.ComplianceEvidence.eicr.status = answerId === "held" ? "held_needs_proof" : answerId === "noProof" ? "weak" : "missing";
  }
  ...
  if (questionId === "tenancyDocs") {
    ...
    brain.ComplianceEvidence.tenancyDocs.status = answerId === "held" ? "held_needs_proof" : "missing";
  }
  ...
  state.unknownIndex = nextApplicableUnknownIndex(Math.min(state.unknownIndex + 1, journeyUnknownQuestions.length), state);
  state.actionPlan = buildJourneyActionPlan(state);
  addTimelineEvent({
  ...
  return `
    <div class="journey-demo-guidance-row" data-demo-guidance-row data-target="${escapeHtml(target)}">
      <aside class="journey-demo-coachmark" data-demo-coachmark data-target="${escapeHtml(target)}">
        <span class="journey-coachmark-header">
          ${guide ? `<img class="cmp-demo-guide-icon" data-demo-macaw-badge src="assets/generated/demo-guidance/macaw-guide-icon.svg" alt="" aria-hidden="true">` : ""}
          <b>${escapeHtml(currentGuidedStory()?.title || "Guided walkthrough")}</b>
        </span>
...
function landingCoachMark() {
  return renderDemoCoachMark(currentGuidedAction());
}

function renderGuidedDemoLanding() {
  const showScenarios = Boolean(guidedDemoState().scenarioExplorerFocus);
  return `
    <section class="journey-guided-landing" aria-labelledby="guidedDemoTitle">
      ...
      <div class="journey-guided-hero">
        <div>
          ...
          <h1 id="guidedDemoTitle">Guided landlord compliance journey</h1>
          <p>Start with one property, run auto checks, review found data, answer unknowns, see CMP's compliance analysis, take one action and finish with monitoring.</p>
          <span class="prototype-badge">Prototype mode · simulated data</span>
          <div class="button-row">
            <button class="primary-button" type="button" data-guided-story="clean-property-check"${guidedTargetAttrs("run-demo")}>Start landlord compliance journey</button>
            <button class="secondary-button" type="button" data-guided-scroll-stories>Try another landlord situation</button>
        ...
        <aside class="journey-guided-vision-card">
          ...
          <p>Add Property → Auto Checks → Review Found Data → Answer Unknowns → Compliance Analysis → Action Plan → Take Action → Monitor & Update → Scenario Explorer.</p>
      ...
      ${showScenarios ? `<section class="journey-guided-story-grid" id="guidedStoryGrid">
        <div class="journey-guided-scenario-intro">
          <p class="section-kicker">Scenario Explorer</p>
          <h2>Try another landlord situation</h2>
          <p>Use these after the main compliance journey lands. Each card changes the landlord situation while preserving the same CMP spine.</p>
        </div>
        ${renderNickScenarioCards()}
      </section>` : ""}
      <section class="journey-guided-simulation-note">
        <strong>What is simulated</strong>
        <p>Smart Search, public-record checks, EPC lookup, licensing signals, document preview, Ask CMP responses, scoring, supplier requests and monitoring are all local prototype state. No live lookup, payment, supplier contact or document storage happens. No live document upload happens; CMP uses Add proof later or evidence gap wording.</p>
      </section>
    </section>
  `;
}

function currentStoryCoachMark() {
  const action = currentGuidedAction();
  if (!action) {
    return "";
  }
  const localTargets = new Set([
    ...
    "open-workspace",
    "open-action-plan",
    "workspace-tab-compliance",
    "workspace-tab-evidence",
    "workspace-tab-services",
...
function renderGuidedPresenterPanel() {
  ...
  const action = currentGuidedAction();
  const skipTarget = action ? "" : guidedTargetAttrs("presenter-skip");
  return `
...
function renderGuidedSectionExplainer(sectionKey) {
  ...
  if (isMainGuidedTour() && isStrictGuidedMode()) {
    return "";
  }
  const sections = {
    actionPlan: {
...
function guidedNextStepsForStory(storyId = guidedDemoState().activeStoryId) {
  const maps = {
    "clean-property-check": [
      ["Prepare legal essentials", "data-journey-service-plan=\"legal\""],
      ["Add missing proof later", "data-journey-open-upload"],
    ...
    "no-epc-found": [
      ["Prepare EPC assessment request", "data-journey-service=\"epc-assessment\" data-service-action=\"booked\""],
      ["Add EPC proof later", "data-journey-open-upload"],
    ...
    "epc-expired-mees-risk": [
      ["Prepare EPC assessment request", "data-journey-service=\"epc-assessment\" data-service-action=\"booked\""],
      ["Prepare EPC/MEES quote request", "data-journey-service=\"epc-improvement-plan\" data-service-action=\"quote_requested\""],
      ["Add current EPC proof later", "data-journey-open-upload"],
    ...
    "hmo-licensing-risk": [
      ["Prepare licensing check request", "data-journey-service=\"licensing-check\" data-service-action=\"booked\""],
      ["Prepare fire risk request", "data-journey-service=\"fire-risk-assessment\" data-service-action=\"booked\""],
    ...
    "damp-mould-enforcement": [
      ["Prepare damp/mould survey request", "data-journey-service=\"damp-mould-survey\" data-service-action=\"booked\""],
      ["Generate tenant message", "data-journey-message=\"damp-photo-request\""],
      ["Add photos/council letter later", "data-journey-open-upload"],
    ...
    "done-for-me-plan": [
      ...
      ["Prepare legal essentials", "data-journey-service-plan=\"legal\""],
      ["Prepare urgent-only request", "data-journey-service-plan=\"urgent\""],
  ...
  return `
    <section class="journey-guided-next-steps">
      <div>
        ...
        <p>The story now returns to the same Property Intelligence profile. Pick one next action to show a request, evidence gap, Ask CMP or monitoring.</p>
      </div>
      <div class="journey-guided-next-grid">
        ${guidedNextStepsForStory(demo.activeStoryId).map(([label, attrs], index) => `
          <button class="secondary-button" type="button" ${attrs}${index === 0 ? guidedTargetAttrs("workspace-action") : ""}>${escapeHtml(label)}</button>
        `).join("")}
      </div>
    </section>
  `;
...
function renderJourneySpine() {
  const state = journeyState();
  const currentStageId = state.currentStage === "start" ? "addProperty" : state.currentStage === "vault" ? "monitor" : state.currentStage;
  const currentIndex = journeyStages.findIndex((stage) => stage.id === currentStageId);
  return `
    <section class="journey-spine-card" aria-label="Guided demo progress">
      <div class="journey-spine-copy">
        <p>Add Property -> Auto Checks -> Review Found Data -> Answer Unknowns -> Compliance Analysis -> Action Plan -> Take Action -> Monitor & Update. Scenario Explorer comes after this route lands.</p>
      </div>
...
function journeyCompletionChecklist(state = journeyState()) {
  const unknownsHandled = state.unknownIndex >= journeyUnknownQuestions.length;
  return [
    { id: "property", label: "Property selected", complete: Boolean(state.selectedMatchId || state.currentStage !== "start") },
    { id: "smartSearch", label: "Auto checks reviewed", complete: state.autoCheckStep >= journeyAutoCheckSteps.length || ["review", "unknowns", "brain", "actionPlan", "takeAction", "monitoring", "workspace"].includes(state.screen) },
    { id: "unknowns", label: "Unknowns answered or marked not sure", complete: unknownsHandled },
    { id: "evidence", label: "Evidence gaps identified", complete: Boolean(Object.values(state.actionPlan || {}).flat().length) },
    { id: "actionPlan", label: "Action plan generated", complete: ["actionPlan", "takeAction", "monitoring", "workspace"].includes(state.screen) || ["actionPlan", "action", "vault", "monitor"].includes(state.currentStage) },
    { id: "service", label: "One evidence/service action taken", complete: Boolean((state.serviceBasket || []).length || (state.evidenceVault || []).length) },
    { id: "ask", label: "Ask CMP opened", complete: state.workspaceTab === "ask" || Boolean((state.askHistory || []).length) },
    { id: "monitor", label: "Monitoring viewed", complete: state.workspaceTab === "monitoring" || state.screen === "monitoring" || state.currentStage === "monitor" }
  ];
}

function shouldGuardJourneyJump(target) {
  const state = journeyState();
  if (!isNickDemoMode() || !guidedDemoState().enabled || state.jumpAheadConfirmed) {
    return false;
  }
  if (!["workspace", "actionPlan"].includes(target)) {
    return false;
  }
  const checklist = journeyCompletionChecklist(state);
  return checklist.some((item) => ["property", "smartSearch", "unknowns"].includes(item.id) && !item.complete);
}

function showJourneyJumpGuard(target) {
  const state = journeyState();
  state.pendingJumpTarget = target;
  state.screen = "missingAnswers";
  state.currentStage = "unknowns";
  renderJourneyOsState();
}
...
function jumpAheadAnyway() {
  const state = journeyState();
  const target = state.pendingJumpTarget || "workspace";
  state.pendingJumpTarget = "";
  state.jumpAheadConfirmed = true;
  if (target === "actionPlan") {
    setJourneyStage("actionPlan", "actionPlan");
    return;
  }
  addTimelineEvent({ title: "Workspace opened early", body: "User jumped ahead before every guided answer was complete.", type: "Guided demo" });
  setJourneyStage("monitor", "workspace");
}

function renderJourneyMissingAnswersGuard() {
  const checklist = journeyCompletionChecklist();
  return renderJourneyShell(`
    <section class="journey-step-panel journey-missing-guard">
      <div class="journey-step-heading">
        <p class="section-kicker">Review missing answers</p>
        <h2>CMP still needs a few answers to make the workspace meaningful</h2>
        <p>You can jump ahead, but the action plan is clearer after Smart Search and landlord-only unknowns are confirmed. "Not sure" is a valid answer and creates an evidence gap.</p>
      </div>
      <div class="journey-checklist-grid">
        ${checklist.map((item) => `
      ...
      <div class="button-row">
        <button class="primary-button" type="button" data-guided-complete-missing>Complete guided steps</button>
        <button class="secondary-button" type="button" data-guided-jump-anyway>Jump ahead anyway</button>
      </div>
    </section>
  `);
}
...
function renderJourneyShell(screenHtml) {
  ...
  return `
    ...
    ${currentStoryCoachMark()}
    <header class="journey-os-header">
      <div>
        <p class="section-kicker">CMP guided journey</p>
        <h1 id="journeyOsTitle">Build a landlord compliance journey</h1>
        <p>Start with an address. CMP checks demo records, asks landlord-only unknowns, analyses compliance, creates an action plan and keeps the property watched.</p>
        <span class="prototype-badge">Prototype mode · simulated data</span>
    ...
    ${renderJourneySpine()}
    ${renderGuidedCallout()}
    ${state.dependencyNotice ? `
      <section class="journey-answer-update-note" role="status">
        <div>
          <strong>Answers updated</strong>
          <span>${escapeHtml(state.dependencyNotice)}</span>
        </div>
      </section>
    ` : ""}
  ...
  return renderJourneyShell(`
    <section class="journey-hero-panel">
      <div>
        ...
        <p>Start with an address. CMP will simulate official checks, ask only for the things public records cannot know, then move into compliance analysis, action and monitoring.</p>
        ...
        <div class="button-row">
          ...
          ${guidedStrict ? "" : `<button class="secondary-button" type="button" data-journey-go="workspace">Open workspace preview</button>`}
        </div>
      </div>
      <aside class="journey-principle-card">
        <strong>No dead ends</strong>
        <p>Every branch returns to the same compliance journey: analysis, action plan, one follow-through path and monitoring.</p>
      </aside>
    </section>
  `);
}
  ...
  return renderJourneyShell(`
    <section class="journey-step-panel">
      ...
      <form class="journey-address-form" data-journey-address-form>
        ...
        <div class="button-row">
          <button class="primary-button" type="submit"${guidedTargetAttrs("run-auto-checks")}>Run simulated auto checks</button>
        ...
        <div class="journey-match-grid">
          ${journeyMatchRecords().map((record) => `
            <article class="journey-match-card">
              ...
              <button class="primary-button" type="button" data-journey-select-match="${escapeHtml(record.id)}"${record.id === "match-1" ? guidedTargetAttrs("confirm-match") : ""}>Select this property</button>
            </article>
          `).join("")}
        </div>
        <div class="button-row">
    ...
    return renderJourneyShell(`
      <section class="journey-step-panel">
        <div class="journey-step-heading">
          ...
          <p>CMP can continue manually, but EPC stays as an action until proof is added later, found or an assessment request is prepared.</p>
        </div>
        <div class="journey-choice-grid">
          ${[
            ...
            ["advertised", "Being advertised"],
            ["vacant", "Vacant / preparing to rent"],
            ["upload", "I have an EPC - add proof later"],
            ["manual", "Continue manually"]
          ].map(([id, label]) => {
            const preferred = guidedDemoState().activeStoryId === "no-epc-found" ? "advertised" : "rented";
            return `<button class="journey-choice" type="button" data-journey-no-epc="${id}"${id === preferred ? guidedTargetAttrs("confirm-match") : ""}>${label}</button>`;
          }).join("")}
        </div>
      </section>
    `);
...
function renderJourneyReview() {
  ...
  const epcMessage = !auto.epcFound
    ? "No clear EPC record found. CMP can continue, but EPC will remain an action until proof is added later, found, or an assessment request is prepared."
    : auto.epcRating === "E"
  ...
  return renderJourneyShell(`
    <section class="journey-step-panel">
      ...
      <div class="button-row">
        ...
        <button class="secondary-button" type="button" data-journey-action="upload" data-action-id="epc-upload">Add proof later</button>
        <button class="text-button" type="button" data-journey-action="book" data-action-id="book-epc">Prepare EPC request</button>
      </div>
    </section>
  `);
}
...
function renderJourneyAnswerReview() {
  ...
  const answered = journeyUnknownQuestions
    .map((question, index) => {
      if (isSkippedJourneyQuestion(question.id, state)) {
        return `
          <button class="journey-answer-chip is-muted" type="button" data-journey-edit-answer="occupancy">
            <span>${escapeHtml(question.title.replace(/\?$/, ""))}</span>
            <strong>Skipped - property is currently vacant</strong>
            <small>Needed later if the property is let</small>
          </button>
        `;
      }
      ...
      const display = journeyQuestionDisplay(question, state);
      const label = answerOptionLabel(question.id, answer);
      return `
        <button class="journey-answer-chip" type="button" data-journey-edit-answer="${escapeHtml(question.id)}">
          <span>${escapeHtml(display.title.replace(/\?$/, ""))}</span>
...
function renderUnknownsWizard() {
  ...
  if (question.id === "condition") {
    ...
    const hasNoKnownIssues = selected.has("none");
    const hasSelectedIssues = Array.from(selected).some((id) => !["none", "unknown"].includes(id));
    const submitCopy = hasNoKnownIssues
      ? "Continue with no known issues"
      : hasSelectedIssues
        ? "Continue with selected issues"
        : selected.has("unknown")
          ? "Continue as not sure"
    ...
    return renderJourneyShell(`
      <section class="journey-step-panel journey-wizard-panel">
        ...
        <div class="button-row">
          ...
          <button class="primary-button" type="button" data-journey-condition-submit${hasSelection ? "" : " disabled"}>${escapeHtml(submitCopy)}</button>
        </div>
        ${renderJourneyAnswerReview()}
      </section>
    `);
...
function renderJourneyBrain() {
  const state = journeyState();
  const complete = state.brainStep >= journeyBrainSteps.length;
  return renderJourneyShell(`
    <section class="journey-step-panel">
      ${complete ? renderComplianceAnalysisReveal() : `
        <div class="journey-step-heading">
          <p class="section-kicker">Property compliance profile</p>
          <h2>CMP is building your property compliance profile</h2>
          <p>Demo analysis is combining simulated records, landlord answers, evidence gaps and service routes.</p>
        </div>
...
function recommendedMainJourneyAction() {
  const plan = journeyState().actionPlan || {};
  return [
    ...(plan.urgentLegalBlockers || []),
    ...(plan.missingEvidence || []),
    ...(plan.expiringSoon || []),
    ...(plan.futureRisks || []),
    ...(plan.improvementOpportunities || [])
  ].find((action) => action.status !== "Deferred") || allJourneyActions()[0] || {
    id: "annual-monitoring",
    title: "Annual compliance monitoring",
    body: "Set monitoring so certificates, evidence gaps and future risks do not disappear.",
    risk: "low",
    status: "Open"
  };
}

function renderCompactActionList(actions = [], emptyCopy = "No issue showing in this bucket.") {
  if (!actions.length) {
    return `<p>${escapeHtml(emptyCopy)}</p>`;
  }
  return `
    <ul class="journey-compact-list">
      ${actions.slice(0, 3).map((action) => `<li><strong>${escapeHtml(action.title)}</strong><span>${escapeHtml(action.body)}</span></li>`).join("")}
    </ul>
  `;
}

function renderComplianceAnalysisReveal() {
  const state = journeyState();
  const brain = state.propertyBrain;
  const plan = state.actionPlan || {};
  const urgent = plan.urgentLegalBlockers || [];
  const missing = plan.missingEvidence || [];
  const future = plan.futureRisks || [];
  const opportunities = plan.improvementOpportunities || [];
  const highRiskCount = [...urgent, ...(plan.conditionRisks || [])].filter((action) => action.risk === "high").length;
  const riskLevel = highRiskCount ? "High" : missing.length || future.length ? "Medium" : "Low";
  const foundRecords = [
    brain.AutoCheckResults.epcFound ? `EPC ${brain.AutoCheckResults.epcRating}` : "No clear EPC record",
    brain.PropertyIdentity.localAuthority,
    brain.PropertyIdentity.identityConfidence
  ];
  const landlordAnswers = Object.keys(state.answers || {}).length;
  return `
    <section class="journey-analysis-reveal" aria-label="Compliance Analysis">
      <div class="journey-step-heading">
        <p class="section-kicker">Compliance Analysis</p>
        <h2>CMP has built the compliance position</h2>
        <p>Found records, landlord answers and evidence gaps now become scores, risk level and a focused action plan.</p>
      </div>
      <div class="journey-progress-grid">
        <article class="journey-workspace-card">
          <span>Input</span>
          <h3>Found records</h3>
          <p>${foundRecords.map(escapeHtml).join(" · ")}</p>
        </article>
        <article class="journey-workspace-card">
          <span>Input</span>
          <h3>Landlord answers</h3>
          <p>${landlordAnswers} answers confirmed and attached to this property profile.</p>
        </article>
        <article class="journey-workspace-card">
          <span>Input</span>
          <h3>Evidence gaps</h3>
          <p>${missing.length ? `${missing.length} proof gap${missing.length === 1 ? "" : "s"} still visible.` : "No missing evidence on this route."}</p>
        </article>
      </div>
      ${renderJourneyScores()}
      <div class="journey-progress-grid">
        <article class="journey-workspace-card is-primary">
          <span>CMP produces</span>
          <h3>Risk level: ${escapeHtml(riskLevel)}</h3>
          <p>Urgent blockers, missing evidence, future risks and opportunities are now ready for the action plan.</p>
        </article>
        <article class="journey-workspace-card">
          <span>CMP produces</span>
          <h3>Urgent blockers</h3>
          ${renderCompactActionList(urgent, "No urgent blocker showing for this route.")}
        </article>
        <article class="journey-workspace-card">
          <span>CMP produces</span>
          <h3>Missing evidence</h3>
          ${renderCompactActionList(missing)}
        </article>
        <article class="journey-workspace-card">
          <span>CMP produces</span>
          <h3>Future risks</h3>
          ${renderCompactActionList(future)}
        </article>
        <article class="journey-workspace-card">
          <span>CMP produces</span>
          <h3>Opportunities</h3>
          ${renderCompactActionList(opportunities, "No improvement opportunity showing yet.")}
        </article>
      </div>
    </section>
  `;
...
function renderMasterActionPlanBuckets() {
  const plan = journeyState().actionPlan || {};
  const buckets = [
    ["Urgent actions / legal blockers", plan.urgentLegalBlockers || [], "No legal blocker showing for this route."],
    ["Expiring soon", plan.expiringSoon || [], "No short-term expiry surfaced yet."],
    ["Missing evidence", plan.missingEvidence || [], "No proof gap showing for this route."],
    ["Future risks", plan.futureRisks || [], "No future risk surfaced yet."],
    ["Opportunities and improvements", plan.improvementOpportunities || [], "No improvement opportunity surfaced yet."]
  ];
  return `
    <section class="journey-master-buckets" aria-label="Action Plan buckets">
      <div class="journey-progress-grid">
        ${buckets.map(([label, actions, emptyCopy]) => `
          <article class="journey-workspace-card">
            <span>${actions.length} item${actions.length === 1 ? "" : "s"}</span>
            <h3>${escapeHtml(label)}</h3>
            ${renderCompactActionList(actions, emptyCopy)}
          </article>
        `).join("")}
      </div>
    </section>
...
function renderJourneyPlanSummary() {
  const state = journeyState();
  const nextAction = recommendedMainJourneyAction();
  const routeLabel = journeyRoutes[state.routeId]?.label || "Prioritised";
  const urgentCount = allJourneyActions().filter((action) => action.risk === "high").length;
  ...
  return `
    <article class="journey-action-card ${action.status === "Deferred" ? "is-deferred" : ""}">
      ...
      <div class="journey-action-buttons">
        <button class="secondary-button" type="button" data-journey-action="upload" data-action-id="${escapeHtml(action.id)}">Add proof later</button>
        <button class="secondary-button" type="button" data-journey-action="book" data-action-id="${escapeHtml(action.id)}">Prepare service action</button>
...
function renderServiceLifecycleButtons(service) {
  ...
  if (status === "recommended") {
    return `
      <button class="primary-button" type="button" data-journey-service="${escapeHtml(service.id)}" data-service-action="booked">Prepare request</button>
  ...
  if (status === "booked") {
    return `
      <button class="primary-button" type="button" data-journey-service-view="${escapeHtml(service.id)}">View request</button>
...
function renderJourneyServiceCard(service) {
  ...
  const nextStep = service.status === "recommended" ? "Open intake or add to basket"
    : service.status === "added" ? "Ready to quote or request"
      : service.status === "quote_requested" ? "Demo quote request is visible in the basket"
        : ["booked", "pending"].includes(service.status) ? "Demo request in progress"
    ...
    <section class="journey-services-shell">
      ...
      <div class="journey-services-summary">
        <article>
          ...
          <small>demo service items</small>
        </article>
        <article>
  ...
  return `
    <section class="journey-evidence-shell">
      ...
      <div class="journey-evidence-grid">
        ...
        `).join("") : `
          <article class="journey-evidence-card"${`${guidedTargetAttrs("evidence-gap")} tabindex="0"`}>
            <h3>No document has been uploaded</h3>
            <p>This demo stores no files. Add proof later, request evidence, or book an assessment to keep the evidence gap visible.</p>
          </article>
        `}
      </div>
    </section>
      ...
      <div class="journey-monitor-grid">
        ${visibleItems.map((item, index) => `
          <article class="journey-monitor-card">
            ...
            <div class="journey-action-buttons">
              <button class="secondary-button" type="button" data-journey-monitor="${escapeHtml(item.id)}" data-monitor-status="watching"${index === 0 ? guidedTargetAttrs("monitoring-item") : ""}>Set reminder</button>
...
function renderJourneyActionPlan() {
  const guidedStrict = isStrictGuidedMode();
  const nextAction = recommendedMainJourneyAction();
  const nextService = journeyServiceForAction(nextAction.id);
  return renderJourneyShell(`
    <section class="journey-step-panel">
      <div class="journey-step-heading">
        <p class="section-kicker">Action Plan</p>
        <h2>Your simulated CMP action plan</h2>
        <p>CMP now turns the compliance analysis into clear buckets and one recommended next action.</p>
      </div>
      ${guidedStrict ? "" : `
      ...
      ${guidedStrict ? "" : renderJourneyScores()}
      ${renderJourneyPlanSummary()}
      ${guidedStrict ? renderMasterActionPlanBuckets() : ""}
      ${guidedStrict ? `
        <section class="journey-plan-controls journey-plan-controls--guided" aria-label="Recommended guided action">
          <p class="section-kicker">Recommended next action</p>
          <h3>${escapeHtml(nextAction.title)}</h3>
          <p>${escapeHtml(nextAction.body)}</p>
          <small>Suggested support: ${escapeHtml(nextService.title)}</small>
          <button class="primary-button is-guided-recommended" type="button" data-guided-take-action${guidedTargetAttrs("action-plan-primary")}>Prepare service request</button>
          <button class="text-button" type="button" data-guided-unlock>Unlock full action plan</button>
        </section>
      ` : ""}
      ${guidedStrict ? `
        <p class="journey-guided-muted-note">More routes, filters and service cards are available after the guided path.</p>
      ` : renderRouteSelector()}
      ${guidedStrict ? "" : renderJourneyPlanButtons()}
      ${guidedStrict ? "" : `
...
function renderGuidedTakeAction() {
  const action = recommendedMainJourneyAction();
  const service = journeyServiceForAction(action.id);
  ...
  return renderJourneyShell(`
    <section class="journey-step-panel">
      ${renderGuidedCoachMarkFor("take-action-primary")}
      <div class="journey-step-heading">
        <p class="section-kicker">Take Action</p>
        <h2>Turn the plan into one practical action</h2>
        <p>CMP can support several follow-through routes, but the main demo follows one clear service request tied to the selected gap.</p>
      </div>
      <article class="journey-workspace-card is-primary">
        <span>Selected action path</span>
        <h3>${escapeHtml(action.title)}</h3>
        <p>${escapeHtml(action.body)}</p>
        <div class="journey-action-insight">
          <strong>Prepare service request</strong>
          <span>${escapeHtml(service.title)} is shown because it links to this property gap.</span>
        </div>
        <button class="primary-button" type="button" data-guided-complete-action${guidedTargetAttrs("take-action-primary")}>Prepare service request</button>
      </article>
      <div class="journey-progress-grid" aria-label="Supporting systems">
        ${supportOptions.map((option) => `
          <article class="journey-workspace-card">
            <span>Supporting system</span>
            <h3>${escapeHtml(option)}</h3>
            <p>${option === "Ask CMP" ? "Guidance can explain the action using this property's facts and evidence." : "Available after the recommended action, without making every option compete at once."}</p>
          </article>
        `).join("")}
      </div>
    </section>
...
function renderGuidedMonitoringStage() {
  const monitoringCards = [
    ["Expiry monitoring", "Certificates and renewal dates stay visible after setup."],
    ["Law change monitoring", "Future landlord rule changes can be surfaced against this property."],
    ["Evidence monitoring", "Missing proof remains tracked until added or resolved."],
    ["Property change monitoring", "Changes to occupancy, condition or use can update the route."],
    ["Supplier follow-up", "Prepared service requests stay connected to the property record."],
    ["Portfolio monitoring later", "The same logic can roll up when more properties are added."]
  ];
  return renderJourneyShell(`
    <section class="journey-step-panel">
      ${renderGuidedCoachMarkFor("monitoring-item")}
      <div class="journey-step-heading">
        <p class="section-kicker">Monitor & Update</p>
        <h2>CMP keeps this property watched.</h2>
        <p>This is the recurring value: the profile does not end at one check or one service request.</p>
      </div>
      <div class="journey-progress-grid">
        ${monitoringCards.map(([title, body], index) => `
          <article class="journey-workspace-card ${index === 0 ? "is-primary" : ""}">
            <span>${index === 0 ? "Final value moment" : "Monitoring"}</span>
            <h3>${escapeHtml(title)}</h3>
            <p>${escapeHtml(body)}</p>
          </article>
        `).join("")}
      ...
      <div class="button-row">
        <button class="primary-button" type="button" data-guided-finish-main${guidedTargetAttrs("monitoring-item")}>Finish main journey</button>
      </div>
    </section>
  `);
}
  ...
  return `
    <section class="journey-workspace-overview">
      <article class="journey-workspace-card is-primary">
        ...
        <div class="button-row">
          <button class="primary-button" type="button" data-journey-action="book" data-action-id="${escapeHtml(topAction?.id || "top-action")}">Prepare service action</button>
          <button class="secondary-button" type="button" data-journey-action="upload" data-action-id="${escapeHtml(topAction?.id || "top-action")}">Add proof later</button>
...
function renderWorkspaceTabCoachMark() {
  const action = currentGuidedAction();
  if (!action?.target?.startsWith("workspace-tab-")) {
    return "";
  }
  return renderDemoCoachMark(action);
}

function journeyStageForWorkspaceTab(tab) {
  if (tab === "evidence") return "vault";
  if (tab === "monitoring") return "monitor";
  if (["services", "timeline", "ask"].includes(tab)) return "action";
  return "brain";
}
  ...
  return renderJourneyShell(`
    <section class="journey-workspace">
      ...
      <nav class="journey-workspace-tabs" aria-label="Journey OS workspace tabs">
        ${tabs.map(([id, label]) => {
          const target = `workspace-tab-${id}`;
          return `<button class="${state.workspaceTab === id ? "is-active" : ""}" type="button" data-journey-workspace-tab="${id}"${guidedTargetAttrs(target)}>${label}</button>`;
        }).join("")}
      </nav>
      ${renderWorkspaceTabCoachMark()}
...
function positionGuidedArrow() {
  // Guided arrows are intentionally disabled. The ring and one tooltip are the guidance surface.
}
...
function renderJourneyOsState() {
  ...
  const screens = {
    ...
    missingAnswers: renderJourneyMissingAnswersGuard,
    brain: renderJourneyBrain,
    actionPlan: renderJourneyActionPlan,
    takeAction: renderGuidedTakeAction,
    monitoring: renderGuidedMonitoringStage,
    ...
    const timer = window.setTimeout(() => {
      ...
      if (index === journeyAutoCheckSteps.length - 1) {
        ...
        const finishTimer = window.setTimeout(() => {
          advanceGuidedMomentToNextScreen("match");
  ...
  journeyBrainSteps.forEach((step, index) => {
    const timer = window.setTimeout(() => {
      ...
      if (index === journeyBrainSteps.length - 1) {
        state.actionPlan = buildJourneyActionPlan(state);
        addTimelineEvent({
          title: "Property compliance profile built",
  ...
  return `
    <section class="journey-intake-modal">
      ...
      <div class="journey-intake-questions">
        ...
        <p>CMP has filled the property context. These are the remaining demo request details.</p>
      ...
      <div class="journey-action-buttons">
        <button class="primary-button" type="button" data-service-intake-complete="${escapeHtml(service.id)}" data-service-final-status="booked">Prepare request</button>
...
function renderFakeUploadModal() {
  ...
  if (!scanner?.documentTypeId) {
    return `
      <section class="journey-upload-modal">
        <h3>Prototype placeholder - no file is uploaded</h3>
        <p>Choose a document type to preview how CMP would classify proof later. Nothing is stored in this prototype.</p>
  ...
  return `
    <section class="journey-upload-modal">
      <h3>${escapeHtml(doc?.label || "Document")} evidence preview</h3>
      <p>Prototype placeholder only. CMP is simulating classification; no file is uploaded or stored.</p>
...
function bindJourneyOs() {
  document.addEventListener("click", (event) => {
    ...
    if (event.target.closest("[data-guidance-hide]")) {
      event.preventDefault();
      guidedDemoState().hintsHidden = true;
      syncGuidedDemoClass();
      renderJourneyOsState();
      return;
    }

    if (event.target.closest("[data-guidance-show]")) {
      event.preventDefault();
    ...
    if (event.target.closest("[data-guided-complete-missing]")) {
      event.preventDefault();
      continueGuidedMissingSteps();
      return;
    }

    if (event.target.closest("[data-guided-jump-anyway]")) {
      event.preventDefault();
      jumpAheadAnyway();
      return;
    }
    ...
    if (goButton) {
      ...
      if (shouldGuardJourneyJump(target)) {
        showJourneyJumpGuard(target);
        return;
      }
      if (target === "add") {
        ...
        setJourneyStage("brain", "workspace");
      }
      if (shouldAdvanceGuided) {
        advanceGuidedProductTarget(guidedTarget, { verifyCurrent: false });
      }
      return;
    }

    const autoReviewButton = event.target.closest("[data-journey-auto-review]");
    if (autoReviewButton) {
      const guidedTarget = clickedGuidedTarget(event);
      const shouldAdvanceGuided = isCurrentGuidedTarget(guidedTarget);
      setJourneyStage("confirmProperty", "match");
    ...
    if (event.target.closest("[data-journey-build-brain]")) {
      ...
      if (shouldAdvanceGuided) {
        advanceGuidedProductTarget(guidedTarget, { applyMoment: false, render: true, verifyCurrent: false });
      }
      return;
    }
    ...
    if (workspaceTab) {
      const state = journeyState();
      state.workspaceTab = workspaceTab.dataset.journeyWorkspaceTab;
      state.currentStage = journeyStageForWorkspaceTab(state.workspaceTab);
      syncGuidedMomentToLiveState();
      renderJourneyOsState();
      return;
    }
    ...
    if (event.target.closest("[data-guided-take-action]")) {
      const guidedTarget = clickedGuidedTarget(event);
      const state = journeyState();
      state.currentStage = "action";
      state.screen = "takeAction";
      syncGuidedMomentToLiveState();
      renderJourneyOsState();
      if (isCurrentGuidedTarget(guidedTarget)) {
        advanceGuidedProductTarget(guidedTarget, { verifyCurrent: false });
      }
      return;
    }

    if (event.target.closest("[data-guided-complete-action]")) {
      completeGuidedMainActionPath();
      return;
    }

    if (event.target.closest("[data-guided-finish-main]")) {
      finishGuidedMainJourney();
      return;
    }
    ...
    if (monitorButton) {
      ...
      if (shouldAdvanceGuided && guidedTarget === "monitoring-item" && isMainGuidedTour()) {
        const demo = guidedDemoState();
        demo.activeStoryId = "";
        demo.activeMomentIndex = 0;
        demo.mode = "landing";
  ...
  document.addEventListener("submit", (event) => {
    ...
    event.preventDefault();
    const guidedTarget = event.submitter?.dataset?.guidedTarget || "";
    const shouldAdvanceGuided = isCurrentGuidedTarget(guidedTarget);
    const state = journeyState();
